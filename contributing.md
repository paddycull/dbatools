...
- Documentation: correcting comment-based help and examples, expanding our examples (more later)
...
You can always [ask a question via GitHub](https://github.com/sqlcollaborative/dbatools/issues/new?assignees=&labels=Question&template=Support_Question.md&title=) as well if you prefer.

## Documentation

Documentation is an area that is a good starting point whether you are new to open-source projects and/or git and GitHub. The documentation focus is around our comment-based help (CBH) that drives our public documentation website: [docs.dbatools.io](https://docs.dbatools.io). CBH is included with every public command (and some internal), and is the content you see using `Get-Help Function-Name` in PowerShell. Reviewing the content to ensure it is consistent and clear on what the command is used for, along with working examples is a key area of discussion. Whether you are first starting out with PowerShell or have been using it for years, your fresh eyes can help spot inaccuracies or areas of improvement on how we document each command. We love making sure we have plenty of examples to illustrate how the commands can be used. If you find anything you can raise an issue to bring it to our attention, then work on a pull request to address it once approved.

## Contributing New Commands

Start out reviewing the [list of functions and documentation](https://docs.dbatools.io), or pulling the list from the module with `Get-Command -Module dbatools -CommandType Function | Out-GridView`. If you find something similar already exists, open [a new issue on GitHub](https://GitHub.com/sqlcollaborative/dbatools/issues/new) to request an enhancement to that command. As well, browse our [Feature](https://github.com/sqlcollaborative/dbatools/labels/Feature) tagged issues to ensure your idea is not already submitted and accepted. If nothing similar pops up, you can *start a new issue for discussion* or ping us in Slack with the details or requirements you need. (_GitHub issue is preferred to have a history of the discussion._)

## Reporting Bugs

[Open a new issue](https://dbatools.io/new-issue/) on GitHub and fill in all the details. The title should report the affected function, followed by a brief description (e.g. _[Bug] Get-DbaDatabase - Add property x to default view_). The provided template holds most of the details we need in order to confirm whether it is reproducible and a potential fix.

## Fix Bugs

We have a [step-by-step guide](https://dbatools.io/firstpull) if you don't know Github enough.
[Open a PR](https://GitHub.com/sqlcollaborative/dbatools/pulls) targeting ideally just one ps1 file (the PR needs to target the *development* branch), with the name of the function being fixed as a title. The maintainers will chime in reviewing the code and either approve the PR or request changes. The more targeted and focused the PR, the easier to merge, the fastest to go into the next release. Keep them as simple as possible to speed up the process.

**Note**: For documentation changes where it effects multiple files, please ensure an issue is present and approved before submitting a large PR.

## Parameters and Variables

As a reference:

- [Parameters](https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_parameters) are at the command level for accepting input (i.e. `Get-DbaDatabase -SqlInstance MyServer` where `SqlInstance` is the parameter)
- [Variable](https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_variables) is used within a given command to store values or objects that are referenced in the workflow of a command's code.
...
Documentation in a command can also come in the format of comments, so if an area of logic is complex or specific to properly handle a situation then comment it up with comment blocks (an [example](https://github.com/sqlcollaborative/dbatools/blob/05fbf9898caf3922d1c9bf8017465bd57e5d387a/functions/Get-DbaDbBackupHistory.ps1#L401)).

## Formatting and indentation

Most of our contributors and maintainers utilize [Visual Studio Code](https://code.visualstudio.com/) now-a-days. We have workspace settings that deal with formatting but this does not catch it all the time. We have since added `Invoke-DbaFormatter` to the project for correcting format to ps1 files. As well, a test is included in our workflow for PRs that validates the formatting is correct. If you should be using another editor of choice you can utilize our formatting command to ensure consistency with our module, and the validation test will pass.
...
- `UnitTests` do not require an instance to be up and running, and are easily the most flexible to be ran on every user computer.
- `IntegrationTests` instead require one or more active instances, and there is a bit of setup to do in order to run them.
